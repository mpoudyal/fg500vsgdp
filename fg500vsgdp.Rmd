...
wd_gdp_plot = function(company) {
  ggplot(wd_gdp) + 
  geom_sf(aes(fill=.data[[company]])) + theme_bw() + 
  ...
  scale_fill_manual(values = c("grey", "orange")) + 
    labs(title = paste0("Countries with GDP lower than annual revenue of"," ",i," in 2019"),
...
# function to create maps for a grid
wd_gdp_plot1 = function(company) {
  ggplot(wd_gdp) + 
  geom_sf(aes(fill=.data[[company]])) + theme_bw() + 
  theme(legend.position = "none",
        plot.title = element_text(colour = "orange", size = 10, face = "bold", hjust = 0.5)) + 
  scale_fill_manual(values = c("grey", "orange")) + 
...
mapgrid + plot_annotation(
  title = "Countries with GDP lower than 2019 annual revenue of ...",
  caption = "Data Sources:\n(i) Fortune Global 500 2019 (https://bit.ly/gf500−2019)\n(ii) IMF World Economic Outlook 2019 (https://bit.ly/imf−weo−2019)",
  theme = theme(plot.title= element_text(colour = "orange", size = 20, face = "bold", hjust = 0.5),
                plot.caption = element_text(colour = "grey", size = 7)))

```

---
[^1]: I downloaded the GDP data from the [IMF World Economic Output database](https://bit.ly/imf-weo-2019).